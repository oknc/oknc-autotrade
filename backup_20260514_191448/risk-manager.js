...
export function getStatus(exchangeId) {
  ...
  try {
    if (fs.existsSync(LOG_FILE)) {
      ...
      for (const entry of allLogs) {
        if (entry.type === 'auto_close' || entry.type === 'strategy_close' || entry.type === 'manual_close') {
          if (entry.realizedPnl !== undefined) {
            if (entry.realizedPnl >= 0) totalWins++;
            else totalLosses++;
          }
        }
      }
  ...
  const totalClosedAll = totalWins + totalLosses;
  const totalWinRate = totalClosedAll > 0
    ? ((totalWins / totalClosedAll) * 100).toFixed(1) + '%'
    : '0%';
  return {
