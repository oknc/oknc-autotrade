...
app.post('/api/cex/strategy/start', async (req, res) => {
  try {
    ...
    const result = await cexStrategy.startStrategy(symbols || ['ETH/USDT', 'BTC/USDT:USDT']);
    if (result.success) appendCexLog('strategy_start', '策略启动 ' + ((symbols || ['ETH/USDT', 'BTC/USDT:USDT']).map(s => s.replace(':USDT','')).join(', ')));
    res.json(result);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
app.post('/api/cex/strategy/stop', (req, res) => {
  try {
    ...
    if (result.success) appendCexLog('strategy_stop', '策略停止');
    res.json(result);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
app.post('/api/cex/exchanges/switch', (req, res) => {
  try {
    res.json(cexEngine.setActiveExchange(req.body.exchangeId));
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
...
app.post('/api/cex/style', (req, res) => {
  try {
    ...
    if (r.success) appendCexLog('style_change', '切换风格: ' + ({conservative:'保守', moderate:'稳健', aggressive:'激进'}[style] || style));
    res.json(r);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
app.post('/api/cex/leverage', async (req, res) => {
  try {
    ...
    if (r.success) appendCexLog('leverage_change', `${symbol} 杠杆: ${leverage}x`);
    res.json(r);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
app.post('/api/cex/position/open', async (req, res) => {
  try {
    ...
    if (result.success) {
      appendCexLog('manual_open', '手动开' + (side === 'long' ? '多' : '空') + ' ' + symbol + ' ' + parseFloat(contracts) + '张');
    } else {
      appendCexLog('manual_open_fail', '开仓失败 ' + symbol + ': ' + (result.error || ''));
    }
    res.json(result);
  } catch (err) {
    appendCexLog('manual_open_fail', '开仓异常 ' + symbol + ': ' + err.message);
    res.status(500).json({ error: err.message });
  }
});
...
app.post('/api/cex/position/close', async (req, res) => {
  try {
    ...
    if (result.success) {
      appendCexLog('manual_close', `手动平${side === 'long' ? '多' : '空'} ${symbol} 盈亏: ${result.realizedPnl || '?'}`);
    }
    res.json(result);
  } catch (err) {
    appendCexLog('manual_close_fail', `平仓失败 ${symbol}: ${err.message}`);
    res.status(500).json({ error: err.message });
  }
});
