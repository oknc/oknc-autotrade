...
import { initEngine, getTargetToken, setTargetToken, getBalances,
         ...
         simulateBuy, simulateSell, checkTokenTax, appendLog } from './engine.js';
import { getActivePrivateKey, getActivePrivateKeyForChain, hasWallets } from './wallet-manager.js';
...
let currentChain = 'bsc';           // 当前链 (bsc/sol)
...
export function setStrategyChain(chain) { currentChain = chain; }
export function getStrategyChain() { return currentChain; }
...
async function executeBatchBuy(candidate, totalAmount, tokenSymbol, splits = 1) {
  if (currentChain === 'sol') {
    return await solEngine.executeSolBuy(candidate.tokenAddress, totalAmount, 100);
  }
  if (splits <= 1) {
    // 不满足分批条件，一次买入
...
export function addPosition(entry, mode = 'auto') {
  const pos = {
    id: `${entry.tokenAddress}_${Date.now()}`,
    tokenAddress: entry.tokenAddress,
    tokenSymbol: entry.tokenSymbol,
    chain: entry.chain || currentChain,
  ...
  logDecision({ type: 'buy', mode, tokenAddress: entry.tokenAddress, tokenSymbol: entry.tokenSymbol, detail: `买入 ${entry.buyAmountBNB} BNB，获得 ${entry.buyAmountToken} 个代币` });
  return pos;
}
...
export async function sellPosition(positionId, percent = 100, mode = 'auto') {
  const pos = positions.find(p => p.id === positionId);
  if (!pos) throw new Error(`持仓不存在: ${positionId}`);
  if (pos.status !== 'active') throw new Error(`持仓状态不是 active: ${pos.status}`);
  const isSol = currentChain === 'sol' || pos.chain === 'sol';
  if (!isSol) await setTargetToken(pos.tokenAddress);
  try {
    pos.status = 'stopping';
    savePositions();
    notifyPositionChange();
    ...
    let actualSell;
    if (isSol) {
      // SOL 持仓：用持仓记录数量，不走 BSC 余额查询
      actualSell = sellAmount;
    } else {
      ...
      actualSell = Math.min(sellAmount, balances.token);
    }
    let result;
    if (isSol) {
      result = await solEngine.executeSolSell(pos.tokenAddress, actualSell, 6, 100);
    } else {
      result = await executeSell(actualSell);
    }
    ...
    logDecision({ type: 'sell', mode, tokenAddress: pos.tokenAddress, tokenSymbol: pos.tokenSymbol, detail: `卖出 ${actualSell} 代币，获得 ${result.amountOut} BNB` });
    // 记录交易日志
    appendLog({
      type: 'sell',
      mode,
      amountIn: actualSell,
      symbolIn: pos.tokenSymbol,
      amountOut: result.amountOut || 0,
      symbolOut: currentChain === 'sol' ? 'SOL' : 'BNB',
...
async function monitorPositions() {
  ...
  try {
    for (const pos of positions) {
      ...
      try {
        let balances, livePrice;
        if (currentChain === 'sol' || pos.chain === 'sol') {
          const jp = await solEngine.getJupiterPrice(pos.tokenAddress);
          livePrice = jp.priceInUSD || 0;
          balances = { bnb: await solEngine.getSolBalance(), usdt: 0, token: await solEngine.getTokenBalance(pos.tokenAddress), tokenSymbol: pos.tokenSymbol };
        } else {
          await setTargetToken(pos.tokenAddress);
          balances = await getBalances();
          livePrice = await getTokenPrice(pos.tokenAddress);
        }
        const priceChange = livePrice || pos.buyPriceUSD;
        // SOL链跳过余额检测（RPC查询常失败，Jupiter报价本身可确认余额）
        const isSolPos = currentChain === 'sol' || pos.chain === 'sol';
        if (!isSolPos && balances.token <= 0 && pos.status === 'active') {
          pos.zeroCount = (pos.zeroCount || 0) + 1;
          if (pos.zeroCount >= 3) {
            pos.status = 'sold'; pos.notes = '链上余额为0'; savePositions(); notifyPositionChange(); continue;
          }
          continue;
        } else if (!isSolPos) {
          pos.zeroCount = 0;
  ...
  for (const candidate of toBuy) {
    ...
    try {
      ...
      appendLog({
        ...
        symbolIn: currentChain === 'sol' ? 'SOL' : 'BNB',
        amountOut: result.amountOut || 0,
        symbolOut: buyCandidate.tokenSymbol,
        txHash: result.txHash || '',
        actualPriceUSD: result.actualPriceUSD || buyCandidate.priceUSD,
      });
...
export async function manualBuy(candidate, amountBNB) {
  ...
  let result;
  if (currentChain === 'sol') {
    result = await solEngine.executeSolBuy(candidate.tokenAddress, buyAmount, 100);
  } else {
    await setTargetToken(candidate.tokenAddress);
    result = await executeBuy(buyAmount);
  }
  addPosition({
  ...
  appendLog({
    ...
    symbolIn: currentChain === 'sol' ? 'SOL' : 'BNB',
    amountOut: result.amountOut || 0,
    symbolOut: candidate.tokenSymbol,
    txHash: result.txHash || '',
    actualPriceUSD: result.actualPriceUSD || candidate.priceUSD,
  });
  return result;
}
...
function logDecision(entry) {
  ...
  if (decisions.length > 2000) decisions = decisions.slice(0, 2000);
  saveDecisions();
}
...
export async function startStrategy() {
  ...
  try {
    if (currentChain === "sol") {
      ...
      if (solPk) {
        try { await solEngine.initSolEngine(solPk); } catch(e) { console.log("[Strategy] SOL init: " + e.message); }
        console.log("[Strategy] SOL钱包: " + (solEngine.getSolWalletAddress() || "").slice(0,10) + "...");
      } else {
        console.log("[Strategy] ⚠ SOL钱包未导入");
      }
    } else {
      if (hasWallets()) { const pk = getActivePrivateKey(); if (pk) { await reinitEngine(pk); console.log("[Strategy] BSC钱包: " + pk.slice(0,10) + "..."); } else throw new Error("无法获取私钥"); }
      else { await initEngine(); }
    }
  } catch (err) { console.error("[Strategy] 初始化失败: " + err.message); throw err; }
  strategyRunning = true; isMonitoring = true;
  console.log("[Strategy] v3 分级已启动 | 链:" + currentChain + " | 持仓:" + positions.filter(p => p.status === "active").length + "/" + MAX_POSITIONS + " | 仿真:" + BUY_ENABLE_SIMULATION + " | 分批:" + BATCH_BUY_ENABLED);
...
export function removePosition(positionId) {
  ...
  savePositions();
  notifyPositionChange();
  console.log('[Strategy] 🗑️ 已删除持仓: ' + removed.tokenSymbol + ' (' + positionId.slice(0,20) + '...)');
  return true;
}
