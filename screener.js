...
import { getNewTokenAddresses, initPairScanner } from './pair-scanner.js';
import { getNewGeckoPairs } from './gecko-scanner.js';

const SEARCH_BASE = 'https://api.dexscreener.com/latest/dex/search';

const SEARCH_QUERIES = [
  'meme', 'pepe', 'doge', 'shib', 'floki', 'bonk', 'trump', 'ai', 'agent', 'agentic',
...
const MAX_AGE_HOURS = parseFloat(process.env.MAX_AGE_HOURS || '72');
...
// ============ 状态 ============
let candidates = [];
let allRawTokens = []; // 全部扫描原始代币（未过滤），供"展开"模式展示
...
export async function fetchLatestPair(tokenAddress, chain) {
  const useChain = chain || currentChain;
  const pairs = await fetchTokenPairs(tokenAddress);
  const allChains = [...new Set(pairs.map(p => p.chainId))];
  const allDexes = [...new Set(pairs.filter(p => p.chainId === CHAIN_CONFIG[useChain].chainId).map(p => p.dexId?.toLowerCase()))];
  const bscPairs = pairs.filter(p =>
    p.chainId === CHAIN_CONFIG[useChain].chainId && CHAIN_CONFIG[useChain].filterDex.includes(p.dexId?.toLowerCase())
  );
  bscPairs.sort((a, b) => parseFloat(b.liquidity?.usd || 0) - parseFloat(a.liquidity?.usd || 0));
  return { pair: bscPairs[0] || null, allChains, allDexes, totalPairs: pairs.length };
}
...
function filterPair(pair, configChain) {
  const pairChain = pair.chainId;
  const dex = (pair.dexId || '').toLowerCase();
  const baseToken = pair.baseToken || {};
  const addr = baseToken.address;

  if (!addr) return { pass: false, reason: '无代币地址' };
  if (pairChain === CHAIN_CONFIG[configChain].chainId && CHAIN_CONFIG[configChain].filterDex.includes(dex) ? false : true) return { pass: false, reason: '非' + CHAIN_CONFIG[configChain].label + '链' };
...
async function performScan() {
  ...
  try {
    ...
    // === 链上 PancakeSwap 新对扫描（仅 BSC） ===
    if (currentChain === 'bsc') {
      try {
        const newTokens = await getNewTokenAddresses();
        if (newTokens.length > 0) {
          console.log(`[Screener] 🔗 链上发现 ${newTokens.length} 个新交易对`);
          let addedFromChain = 0;
          for (const tokenAddr of newTokens) {
            try {
              const pairs = await fetchTokenPairs(tokenAddr);
              for (const p of pairs) {
                if (p.chainId !== CHAIN_CONFIG[currentChain].chainId || !CHAIN_CONFIG[currentChain].filterDex.includes(p.dexId?.toLowerCase())) continue;
                const key = p.baseToken?.address?.toLowerCase();
                if (!key) continue;
                if (!allPairs.has(key) || parseFloat(p.liquidity?.usd || 0) > parseFloat((allPairs.get(key)?.liquidity?.usd) || 0)) {
                  allPairs.set(key, p);
                  addedFromChain++;
                }
              }
          ...
          if (addedFromChain > 0) console.log(`[Screener] 🔗 链上扫描新增 ${addedFromChain} 个新代币`);
        }
      } catch (err) {
        console.error(`[Screener] ⚠️ 链上扫描异常: ${err.message}`);
      }
    }

    // === GeckoTerminal 新池子扫描（仅 BSC） ===
    if (currentChain === 'bsc') {
      try {
        const geckoPools = await getNewGeckoPairs();
        if (geckoPools.length > 0) {
          console.log(`[Screener] 🦎 GeckoTerminal 发现 ${geckoPools.length} 个新池`);
          let addedFromGecko = 0;
          for (const { tokenAddress } of geckoPools) {
            try {
              const pairs = await fetchTokenPairs(tokenAddress);
              for (const p of pairs) {
                if (p.chainId !== CHAIN_CONFIG[currentChain].chainId || !CHAIN_CONFIG[currentChain].filterDex.includes(p.dexId?.toLowerCase())) continue;
          ...
          if (addedFromGecko > 0) console.log(`[Screener] 🦎 GeckoTerminal 新增 ${addedFromGecko} 个代币`);
        }
      } catch (err) {
        console.error(`[Screener] ⚠️ GeckoTerminal 异常: ${err.message}`);
      }
    }
    ...
    // 保存原始扫描结果（前50个未过滤的代币），供"展开"模式
    const TOP_RAW = 50;
    allRawTokens = sortedPairs.slice(0, TOP_RAW).map(p => ({
      ...
      buySellRatio: calcRatio((p.txns?.h24?.buys || 0), (p.txns?.h24?.sells || 0)),
      marketCap: parseFloat(p.marketCap || 0),
      fdv: parseFloat(p.fdv || 0),
      chain: currentChain,
      dexUrl: `https://dexscreener.com/${CHAIN_CONFIG[currentChain]?.dexUrlPrefix || 'solana'}/${p.baseToken?.address || ''}`,
    ...
    for (const pair of sortedPairs) {
      ...
      const result = filterPair(pair, currentChain);
...
export function getAllRawTokens() { return allRawTokens; }
export function removeCandidate(tokenAddress) {
  const before = candidates.length;
  candidates = candidates.filter(c => c.tokenAddress !== tokenAddress);
  console.log(`[Screener] 已移除候选: ${tokenAddress} (${before} → ${candidates.length})`);
  return { removed: before > candidates.length };
}
 ...
 * @returns {object} 分析报告
 */
export async function analyzeToken(tokenAddress, chain) {
  try {
  const useChain = chain || currentChain;
  const fetchResult = await fetchLatestPair(tokenAddress, useChain);
  const pair = fetchResult.pair;
    if (!pair) {
      let detail = '';
      if (fetchResult.totalPairs > 0 && fetchResult.allChains && fetchResult.allChains.length > 0) {
        detail = ' (DexScreener找到' + fetchResult.totalPairs + '个交易对，链: ' + fetchResult.allChains.join('/');
        var cfg = CHAIN_CONFIG[useChain];
        if (fetchResult.allChains.indexOf(cfg.chainId) >= 0) {
          detail += '，' + cfg.label + '上DEX: ' + (fetchResult.allDexes.join(', ') || '无');
          if (fetchResult.allDexes.length > 0) detail += '，但不在' + (cfg.filterDex.join(', ')) + '上';
        }
        detail += ')';
      }
      return { error: '未找到 ' + CHAIN_CONFIG[useChain].label + ' DEX 交易对' + detail };
    }

    // 运行筛选
    const filterResult = filterPair(pair, useChain);
    ...
    const dexUrl = `https://dexscreener.com/${CHAIN_CONFIG[useChain].dexUrlPrefix}/${tokenAddress}`;
